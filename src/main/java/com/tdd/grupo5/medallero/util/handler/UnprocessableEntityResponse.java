...
public class UnprocessableEntityResponse extends ErrorResponse {
  ...
  public UnprocessableEntityResponse(String entityName, Set<Validation> validations) {
    super(HttpStatus.UNPROCESSABLE_ENTITY, "validation_error", String.format("Invalid entity %s.", entityName));
  ...
  public String getValidationErrorsMessage() {
    return (String)this.validations.stream().map((v) -> {
      return "[" + this.entityName + "." + v.getProperty() + ": " + v.getMessage() + "]";
